...
test("release workflow publishes npm packages from release tags", async () => {
  ...
  assert.match(workflow, /Check npm publication/);
  assert.match(workflow, /npm view "agentic-workflow-guard@\$VERSION" version --json/);
  assert.match(workflow, /Require NPM_TOKEN for real publish/);
  assert.match(workflow, /steps\.npm\.outputs\.published != 'true'/);
  ...
  assert.match(workflow, /Skip npm publish/);
