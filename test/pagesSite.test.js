...
test("docs:build creates a Pages-ready site with stable schema URLs", async () => {
  ...
  const configSchema = JSON.parse(await readFile(path.join(outDir, "schemas", "config.schema.json"), "utf8"));
  ...
  const ciPack = JSON.parse(await readFile(path.join(outDir, "rules", "community", "agentic-workflow-guard-ci-pipeline-hardening.json"), "utf8"));
  ...
  assert.equal(configSchema.$id, "https://guorunjie.github.io/agentic-workflow-guard/schemas/config.schema.json");
  ...
  assert.ok(schemaIndex.schemas.some((schema) => schema.path === "schemas/config.schema.json"));
  ...
  assert.ok(registry.packs.some((pack) => pack.alias === "ci-pipeline-hardening"));
  ...
  assert.ok(ciPack.platforms.includes("teamcity"));
  assert.ok(ciPack.platforms.includes("harness"));
  assert.ok(ciPack.platforms.includes("tekton"));
  assert.ok(ciPack.platforms.includes("argo-workflows"));
  assert.ok(ciPack.platforms.includes("aws-codebuild"));
  assert.ok(ciPack.platforms.includes("google-cloud-build"));
