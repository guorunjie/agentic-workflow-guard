...
- Use a release tag such as `v0.5.0` in docs and examples.
- Keep SARIF output as the default Action output path.
- Include README examples for local CLI, GitHub Action, SARIF upload, baseline mode, `fix --patch`, `fix --apply`, config, `rules verify`, `benchmark`, `mcp resources`, browser traces, and agent support.
