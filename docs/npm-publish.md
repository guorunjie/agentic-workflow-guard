...
Mention scanner coverage, policy profiles, suppression audit trails, stable report, fix, and rule pack schemas, `scan --output`, `fix --format json` approval snippets, trusted rule marketplace metadata, community rule-pack registry, MCP resources, Skillpack Forge output, GitHub Action usage, and benchmark status. Keep the npm package aligned with the GitHub release tag.
