...
async function packageGate(root) {
  ...
  const requiredScripts = ["test", "docs:build", "smoke:package", "release:check", "release:prepare", "release:status", "release:publish", "release:verify", "release:sync", "release:sync:check", "benchmark:report", "benchmark:corpus", "mcp:resources", "schema:config", "doctor"];
  const missingScripts = requiredScripts.filter((script) => !pkg.scripts?.[script]);
  const requiredFiles = ["bin", "src", "rules", "schemas", "mcp", "examples", "benchmarks", "docs", "docs-site", "scripts", "action.yml", "ROADMAP.md", "CONTRIBUTING.md", "SECURITY.md", "CODE_OF_CONDUCT.md", ".github/copilot-instructions.md", ".github/pull_request_template.md", ".github/ISSUE_TEMPLATE"];
