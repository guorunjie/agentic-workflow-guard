...
export async function scanCiWorkflows(root) {
  ...
  for (const file of files) {
    ...
    if (hasAgent && platform === "AWS CodeBuild") {
      const credentials = [...text.matchAll(codeBuildCredentialPattern)];
      for (const credential of credentials) {
        findings.push(makeFinding("AWI007", `${file}:${lineOf(text, credential.index)}`, `AWS CodeBuild secret ${credential[2].trim()} from ${credential[3].trim()} is attached to an agent build`));
      }
    }

    if (hasAgent && platform === "Google Cloud Build") {
